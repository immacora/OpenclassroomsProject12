...
class TestGetClient:
    """
    GIVEN fixtures for client (with location and sales contact with tokens)
    and employees with their associated users and tokens
...
class TestPutClient:
    ...
    GIVEN fixtures for client (with location and sales contact with tokens)
    and employees with their associated users and tokens
    WHEN user tries to update client informations and locations
    or update client sales_contact or add contract to client
    THEN checks that the response is valid and data are displayed
    """
    ...
    def test_put_client_informations_route_success(self, api_client, new_client):
        """
        GIVEN an existing client with a location, his sales contact valid token,
        and valid data
    ...
    def test_put_client_remove_location_route_success(self, api_client, new_client):
        """
        GIVEN an existing client with a location, his sales contact valid token,
        and client location_id
        WHEN the client endpoint is updated to (PUT)
        THEN checks that response is 200 and datas are displayed
        """
        valid_client_location_id = new_client.locations.values_list(
            "location_id", flat=True
        ).first()
        valid_client_data = {"location_id": valid_client_location_id}
        access_token = new_client.sales_contact.user.access_token
        headers = {"Authorization": f"Bearer {access_token}"}
        client_id = new_client.client_id
        response = api_client.put(
        ...
        assert Client.objects.count() == 1
        assert Location.objects.count() == 0
        assert "location_id" not in response.data["locations"]

    def test_put_client_remove_location_route_failed_with_wrong_uuid_bad_request(
        self, api_client, new_client
    ):
        """
        GIVEN an existing client with a location, his sales contact valid token,
        and uuid witch is not client location_id
        WHEN the client endpoint is updated to (PUT)
        THEN checks that response is 400 and error message is displayed
        """
        invalid_client_data = {"location_id": "f37790ce-a3c1-474b-9e8a-b7a959a80b8e"}
        access_token = new_client.sales_contact.user.access_token
        headers = {"Authorization": f"Bearer {access_token}"}
        client_id = new_client.client_id
        response = api_client.put(
        ...
        assert Client.objects.count() == 1
        assert Location.objects.count() == 1
        assert "Le lieu n'existe pas." in response.data["details"]

    def test_put_client_remove_location_route_failed_with_no_uuid_bad_request(
        self, api_client, new_client
    ):
        """
        GIVEN an existing client with a location, his sales contact valid token,
        ...
        THEN checks that response is 400 and error message is displayed
        """
        invalid_client_data = {"location_id": "WRONG"}
        access_token = new_client.sales_contact.user.access_token
        headers = {"Authorization": f"Bearer {access_token}"}
        client_id = new_client.client_id
        response = api_client.put(
        ...
        assert Client.objects.count() == 1
        assert Location.objects.count() == 1
        assert "Veuillez saisir un UUID valide." in response.data["details"]
    ...
    def test_put_client_sales_contact_route_failed_with_no_sales_contact_id_bad_request(
        ...
        """
        GIVEN an existing client with a location, his sales contact valid token,
        and employee id witch is not sales_contact
        WHEN the client endpoint is updated to (PUT)
        THEN checks that response is 400 and error message is displayed
        """
    ...
    def test_put_client_sales_contact_route_failed_with_invalid_id_bad_request(
        ...
        """
        GIVEN an existing client with a location, his sales contact valid token,
        and invalid updated_sales_contact
        WHEN the client endpoint is updated to (PUT)
        THEN checks that response is 400 and error message is displayed
        """
        invalid_client_data = {
    ...
    def test_put_client_add_contract_route_success(
        self, api_client, new_client, employees_users_with_tokens
    ):
        """
        GIVEN an existing client with a location, his sales contact valid token,
        and True boolean for add_contract field
        WHEN the client endpoint is updated to (PUT)
        THEN checks that response is 200 and datas are displayed
        """
        new_client.contract_requested = True
        new_client.save()
        valid_client_data = {"add_contract": "True"}
        access_token = employees_users_with_tokens[
            "management_employee"
        ].user.access_token
        headers = {"Authorization": f"Bearer {access_token}"}
        ...
        assert "payment_due" in response.data
        assert response.data["is_signed"] is False
        assert "client" in response.data
        assert "created_at" in response.data
        assert "updated_at" in response.data

    def test_put_client_add_contract_route_with_unauthorized(
        self, api_client, new_client, employees_users_with_tokens
    ):
        """
        GIVEN an existing client with a location, his sales contact valid token,
        and True boolean for add_contract field
        WHEN the client endpoint is updated to (PUT) but contract not requested
        THEN checks that response is 401 and error message is displayed
        """
        valid_client_data = {"add_contract": "True"}
        access_token = employees_users_with_tokens[
...
class TestDeleteClient:
    ...
    def test_delete_client_route_with_signed_contract_unauthorized(
        ...
        """
        GIVEN a contract and his sales contact client valid token
